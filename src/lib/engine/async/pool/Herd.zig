...
stack_size: u32,
max_threads: u32,
atm_threads_stack: Atomic(?*Thread) = Atomic(?*Thread).init(null),
threads_stack: std.ArrayList(*Thread) = undefined,
...
pub fn init(target: *ThreadRipper, options: Options) !void {
    ...
    target.* = .{
        ...
        .arena = options.arena,
        .threads_stack = std.ArrayList(*Thread).init(options.arena),
    };
...
fn register(noalias tr: *ThreadRipper, noalias thread: *Thread) void {
    // Push the thread onto the threads stack in a lock-free manner.
    var atm_threads_stack = tr.atm_threads_stack.load(.monotonic);
    try tr.threads_stack.append(thread);
    // Threads is a stack we add the thread to the front of the stack
    while (true) {
        thread.next = atm_threads_stack;
        atm_threads_stack = tr.atm_threads_stack.cmpxchgStrong(
            atm_threads_stack,
            thread,
            .release,
            .monotonic,
...
fn createNodeWithCtx(tr: *ThreadRipper, comptime func: anytype, args: anytype) !*Node {
    ...
    const Closure = struct {
        ...
        fn runFn(action: *Action, ctx: ?*Context) !void {
            ...
            // print("{any}\n", .{ctx.?.id});
            try @call(.auto, func, .{ closure.arguments[0], closure.arguments[1], closure.arguments[2], ctx.? });
...
const Thread = struct {
    ...
    fn pop(noalias thread: *Thread, noalias tr: *ThreadRipper) ?ActionQueue.Thief {
        ...
        while (num_threads > 0) : (num_threads -= 1) {
            // Traverse the stack of registered threads on the thread pool
            const target = thread.target orelse tr.atm_threads_stack.load(.acquire) orelse unreachable;
            // if (target == null) return null;
            thread.target = target.next;
